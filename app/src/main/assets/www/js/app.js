...
const App = {
  ...
  init() {
    ...
    $("#quoteBtn").addEventListener("click", () => {
      ...
      this.renderSheet();   // rebuild content (empty state offers the ✨ AI button)
      const sheet = $("#sheet");
  ...
  async expRun(source) {
    ...
    if (missing) {
      ...
      let base = !val ? ""
        : r.via === "course" ? "From your course — instant"
        : r.via === "dict" ? (online ? "Built-in dictionary — instant" : "Pocket dictionary — works offline")
      ...
      const hasEx = !!(r.examples && r.examples.length);
      if (hasEx) {
        base += r.examples.some(x => x.src === "ai")
          ? " · 💬 examples ready — tap ✨ for fresh AI sentences"
          : online ? " · 💬 tap for examples — ✨ writes new ones"
            : " · tap 💬 for a real example sentence";
      } else if (online) {
        base += " · getting example sentences…";
      } else {
        base += " · no example sentence found for this one — the translation is still solid";
      }
      this.exp.status = base;
      // sentences stream in AFTER the translation is already on screen
      if (online) {
        const myToken = token;
        const baseDone = hasEx ? null : base.replace(" · getting example sentences…", "");
        Translate.loadExamples(r).then(nr => {
          if (myToken !== this.reqToken || !nr) return;
          this.exp.result = nr;
          const ex = nr.examples || [];
          let msg = baseDone === null ? this.exp.status : baseDone;
          if (baseDone !== null) {
            if (ex.length) {
  ...
  action(act, d, t) {
    switch (act) {
      ...
      case "freshEx": {
        ...
        const st = $("#expStatus");
        if (t) { t.classList.add("spin"); t.disabled = true; }
